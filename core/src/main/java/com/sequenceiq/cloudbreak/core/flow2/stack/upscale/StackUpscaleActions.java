    ...
    public Action<?, ?> prevalidate() {
        return new AbstractStackUpscaleAction<>(StackScaleTriggerEvent.class) {
            ...
            protected void prepareExecution(StackScaleTriggerEvent payload, Map<Object, Object> variables) {
                variables.put(HOST_GROUP_WITH_ADJUSTMENT, payload.getHostGroupWithAdjustment());
                variables.put(HOST_GROUP_WITH_HOSTNAMES, payload.getHostGroupWithHostnames());
            ...
            protected void doExecute(StackScalingFlowContext context, StackScaleTriggerEvent payload, Map<Object, Object> variables) {
                Map<String, Integer> hostGroupWithAdjustment = payload.getHostGroupWithAdjustment();
                int instanceCountToCreate = 0;
                //TODO ki kell faszazni, ne legyen ennyi iteracio
                for (String hostGroup : hostGroupWithAdjustment.keySet()) {
                    Integer adjustment = hostGroupWithAdjustment.get(hostGroup);
                    instanceCountToCreate += stackUpscaleService.getInstanceCountToCreate(context.getStack(), hostGroup,
                            adjustment, context.isRepair());
                }

                stackUpscaleService.addInstanceFireEventAndLog(context.getStack(), hostGroupWithAdjustment, payload.getAdjustmentTypeWithThreshold());
                if (instanceCountToCreate > 0) {
                    stackUpscaleService.startAddInstances(context.getStack(), hostGroupWithAdjustment);
                    sendEvent(context);
                } else {
            ...
            protected Selectable createRequest(StackScalingFlowContext context) {
                ...
                Map<String, Integer> hostGroupWithAdjustment = context.getHostGroupWithAdjustment();
    ...
    public Action<?, ?> stackUpscaleFailedAction() {
        return new AbstractStackFailureAction<StackUpscaleState, StackUpscaleEvent>() {
            ...
            protected void doExecute(StackFailureContext context, StackFailureEvent payload, Map<Object, Object> variables) {
                ...
                // TODO tesztelni kell ezt az agat, lehet nem tul jo igy
