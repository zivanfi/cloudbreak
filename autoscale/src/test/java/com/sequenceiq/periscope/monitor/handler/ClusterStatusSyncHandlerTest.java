...
public class ClusterStatusSyncHandlerTest {
    ...
    @Test
    public void testOnApplicationEventWhenEnvironmentIsNotInitialized() {
        Cluster cluster = getACluster(ClusterState.RUNNING);
        cluster.setEnvironmentCrn(null);
        ...
        when(cloudbreakCommunicator.getAutoscaleClusterByCrn(anyString())).thenReturn(autoscaleStackV4Response);
        when(autoscaleStackV4Response.getEnvironmentCrn()).thenReturn("testEnvironment");

        underTest.onApplicationEvent(new ClusterStatusSyncEvent(AUTOSCALE_CLUSTER_ID));

        verify(clusterService).setEnvironmentCrn(AUTOSCALE_CLUSTER_ID, "testEnvironment");
        verify(cloudbreakCommunicator).getStackStatusByCrn(CLOUDBREAK_STACK_CRN);
        verify(cloudbreakCommunicator).getAutoscaleClusterByCrn(CLOUDBREAK_STACK_CRN);
    }

    @Test
    public void testOnApplicationEventWhenMachineUserIsNotInitialized() {
        Cluster cluster = getACluster(ClusterState.RUNNING);
        ...
        verify(cloudbreakCommunicator).getStackStatusByCrn(CLOUDBREAK_STACK_CRN);
        verify(altusMachineUserService).initializeMachineUserForEnvironment(cluster);
    }
